...
├── .gitignore
├── README.md
├── TestPlan_SauceDemo.docx     # Full test plan document
...
- **Manual state reset:** Cart state persists between sessions and must be cleared manually between test runs if needed. `SauceDemo resets state can be done manually` — there is no automatic reset on login.
